...
2. Develop a hypothesis 
    a. Methodology (map out your plan)
    b. Clean data, what variables you want to work with
...
6. Finalize model (train on whole dataset)

7. Interpret results, visualize, communicate zzzz
...
• lots of NA in the cabin column, probably just eliminate it from model consideration, ticket number also probably unecessary 
• age could be formatted to not have 00 
• things that are likely to be relevant for model selection include class, sex, age, and fare 

```{r}
t_df_EDA <- t_df |> 
  select(passenger_id, survived, pclass, name, sex, age, fare, sib_sp, parch)|>
  drop_na()
...
mean(survivor$fare) #51.84
mean(dead$fare) #22.97 

TitanicBox <- ggplot(t_df_EDA, aes(y=fare, x=survived, group=survived))+
  geom_boxplot()+
  labs(x="Survived", y="Fare")+
  scale_y_continuous(0:550)
  theme_bw()
TitanicBox

TitanicBar <- ggplot(t_df_EDA, aes(x=pclass, group =survived, fill = survived))+
  geom_histogram()
TitanicBar
...
```
Variables of Interest: class, age, sex, embark, fare? 
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>%
  update_role(passenger_id, new_role = "ID") %>% #not an independent variable just an ID 
  step_rm(name,age) %>% #too many NA's not worth running 
  step_unknown(all_nominal(),-all_outcomes()) |> #if you see NA, make new factor of unknown 
  step_dummy(all_nominal(),-all_outcomes()) |> #all categories are dummy variables 
  step_zv(all_numeric(),-all_outcomes()) |> #any with no variance, get rid of them 
  step_normalize(all_numeric(),-all_outcomes()) #all numeric data must be normalized, all same scale 
...
Lasso is barely better than the logistic model 
...
lasso_fit |>
  extract_fit_parsnip() |> 
  tidy() #ones we dont need are set to 0 
```
